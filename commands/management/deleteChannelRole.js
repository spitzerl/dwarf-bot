...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!channelDataToDelete) {
			return interaction.reply({ content: "Ce channel n'existe pas.", flags: 64 });
		}
		...
		try {
			...
			interaction.reply({ content: `Channel et rôle supprimés avec succès !`, flags: 64 });
		} catch (error) {
			console.error(error);
			interaction.reply({ content: "Erreur lors de la suppression du channel et du rôle.", flags: 64 });
		}
	},
};
